 ...
 * 1. Checks local cache first for instant results
 * 2. If not cached, queues API request to prevent rate limits
 * 3. Retries on server errors only (5xx)
 */
export async function analyzeNutrition(
  ...
  if (cachedResult) {
    if (config.debugMode) {
      console.log('Cache hit for:', mealText);
    }
    return cachedResult;
  }

  // Step 2: If not cached, use request queue to prevent rate limits
  if (config.debugMode) {
